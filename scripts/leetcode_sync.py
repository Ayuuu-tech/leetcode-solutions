...
# Everything is reckoned in IST, not the runner's UTC clock, so a problem
# solved at 00:30 IST counts for the right day.
...
SESSION_HELP = (
    ...
    "Fix: open leetcode.com (logged in) > DevTools (F12) > Application > Cookies >\n"
    "https://leetcode.com, copy the LEETCODE_SESSION value, then update the repo\n"
    "secret at Settings > Secrets and variables > Actions.\n"
    "This cookie expires roughly monthly - it is the only recurring maintenance here."
)
...
def main():
    ...
    todays, seen = [], set()
    for s in recent:
        d = datetime.datetime.fromtimestamp(int(s["timestamp"]), IST).date()
